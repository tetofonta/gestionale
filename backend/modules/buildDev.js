...
fs.readdirSync(".").map(name => `./${name}`).filter(isDirectory).filter(startsWith("module_")).forEach(modulePath => {
    ...
    webpack({
        ...
        if (err || stats.hasErrors()) {
            ...
            fs.appendFileSync(path.resolve(__dirname, `./built/${infos.name}.js`),
                `

/****************************************************************************/
/***NB: THIS COSE WAS APPEND BY THE COMPILER TO WARN ABOUT DEV ENVIRONMENT***/
if (!process.env.NODE_ENV || process.env.NODE_ENV === 'development') {
    console.warn("WARNING: USING COMPILED CODE FOR DEVELOPMENT OR TESTING")
}
/****************************************************************************/
        `
            );
